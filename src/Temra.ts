export declare type ReplaceFunction<E = unknown, O = unknown> = (e: Element, optionals?: TransformationOptionals<E, O>) => Promise<string> | string;
...
export declare interface TransformationOptionals<E = unknown, O = unknown> {
	env?: E,
	req?: Request,
	other?: O;
}
 ...
 */
export class Temra<E = unknown, O = unknown> {
	/**
	 ...
	 * ```
	 */
	add<E = unknown>(selector: string, replacerFunction?: ReplaceFunction<E, O>, options?: AddOptions): Temra {
	...
	transform(body: Response, optionals?: TransformationOptionals<E, O>): Response {
		...
		if (this.deleteComments) rewriter.onDocument({
			comments: (c: Comment) => {
				c.remove();
			}
		});
